...
const PROFILE_POOL = [
  "喜歡在清晨散步，講話不快，但觀察別人的細節很仔細。",
  "平常是手作甜點愛好者，對味道和情緒變化都很敏感。",
  "偏內向，遇到不熟的人會先聽再說，但熟了之後很會接話。",
  "喜歡露營和看星空，面對壓力時會故作冷靜，其實很容易想太多。",
  "是重度音樂愛好者，常用旋律和節奏形容自己的心情。",
  "平常愛記帳、做規劃，遇到混亂場面會下意識整理脈絡。",
  "喜歡逛展覽與拍照，對人的表情和語氣特別敏感。",
  "自認幽默感不錯，緊張時會故意講得輕鬆一點。",
  "習慣從生活小事判斷一個人，直覺強但不喜歡太快下結論。",
  "做事講求效率，但在朋友面前其實很願意陪聊和傾聽。",
  "喜歡看球賽和熱鬧場合，直來直往但不想讓氣氛太僵。",
  "偏好閱讀社會議題文章，說話會習慣先想理由再表態。",
  "是咖啡店常客，喜歡邊觀察路人邊猜別人的故事。",
  "很重視誠意，對太完美或太制式的說法會本能起疑。",
  "熱愛桌遊，對規則和話術都很有感，也常注意誰在帶風向。",
  "容易因為對方語氣改變而提高警覺，但不愛直接翻臉。 "
];
...
function assignVirtualIdentities(room) {
  ...
  const shuffledProfiles = shuffle(PROFILE_POOL);
  ...
  orderedPlayers.forEach((player, index) => {
    ...
    player.profile = shuffledProfiles[index % shuffledProfiles.length].trim();
  });
...
function scheduleRoomTask(room, callback, delay = 1200) {
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      addMessage(room, {
        type: "system",
        content: "系統處理 AI 行動時發生問題，已改用安全備援流程。"
      });
      emitRoomState(room);
    }
  }, delay);
...
5. 字數控制在 80 到 180 字，最多不得超過 220 字。
...
function fallbackAiSpeech(player, round) {
  const base = [
    `我是${getDisplayName(player)}，${player.profile}`,
  ...
  if (round.id === "round_4") {
    return `到這輪我還是維持原本的節奏，因為硬演反而更怪。${player.profile}這點其實跟我剛剛幾輪的說法都有接上，我比較希望大家看一個人前後是不是自然，不是只看誰講得漂亮。`;
  }
...
function markEliminated(room, playerId, reason) {
  ...
  addMessage(room, {
    type: "system",
    content: `${getDisplayName(player)} 已遭淘汰。原因：${reason}`
  });
}
...
function finalizeGame(room, winner) {
  ...
  addMessage(room, {
    type: "system",
    content: `${winner.title}。${winner.detail}`
  });
  addMessage(room, {
    type: "system",
    content: `最終身分公開：${revealText}`
  });
}
...
function beginRound(room) {
  ...
  addMessage(room, {
    type: "system",
    content: getRoundAnnouncement(room)
  });
